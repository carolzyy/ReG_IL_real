...
RL_TRAIN_FORMAT = [
    ("step", "S", "int"),
    ("episode", "E", "int"),
    ("critic_loss", "Lc", "float"),
    ("bc_loss", "Lbc", "float"),
    ("q_mean", "q", "float"),
    ("reward", "R", "float"),
    ("reward_gt", "R_gt", "float"),
...
RL_EVAL_FORMAT = [
    ...
    ("episode", "E", "int"),
    ("episode_length", "L", "int"),
    ("episode_reward_gt", "R_gt", "float"),
    ("episode_reward", "R", "float"),
    ("success_rate", "SR", "float"),
    ("retrieve_rate", "ReR", "float"),
    ("total_time", "T", "time"),
]
...
class MetersGroup(object):
    def __init__(self, csv_file_name, formating):
        ...
        self._fieldnames = [f[0] for f in formating] + ["frame"]
    ...
    def _dump_to_csv(self, data):
        if self._csv_writer is None:
            ...
            self._csv_writer = csv.DictWriter(
                self._csv_file, fieldnames=sorted(data.keys()), restval=0.0,extrasaction='ignore'
            )
    ...
    def __init__(self, log_dir, use_tb, mode='bc'):
        ...
        if mode == "bc":
            self._train_mg = MetersGroup(
                log_dir / "train_bc.csv", formating=BC_TRAIN_FORMAT
            )
            self._eval_mg = MetersGroup(log_dir / "eval_bc.csv", formating=BC_EVAL_FORMAT)
        elif mode == "rl":
            self._train_mg = MetersGroup(
                log_dir / "train_rl.csv", formating=RL_TRAIN_FORMAT
            )
            self._eval_mg = MetersGroup(
                log_dir / "eval_rl.csv", formating=RL_EVAL_FORMAT
            )
