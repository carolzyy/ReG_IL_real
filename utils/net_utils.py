...
class TanhNormal(object):
    """
    A proper SAC-compatible tanh-squashed Gaussian distribution.
    Implements:
    ...
    def __init__(self, loc, scale, eps=1e-6):
        ...
        self.scale = scale
        self.eps = eps
        self.normal = pyd.Normal(loc, scale)

    def rsample(self):
        """Reparameterized sample: a = tanh(loc + scale*eps)"""
        z = self.normal.rsample()          # [B, action_dim]
        action = torch.tanh(z)
        return action, z

    def sample(self):
        """Non-reparameterized sample (rarely used in SAC, mainly for eval mode)."""
    ...
    def log_prob(self, action, pre_tanh_value=None):
        """
        Computes:
            logp = logN(z | loc, scale) - sum log(1 - tanh(z)^2 + eps)
        where:
            z = atanh(action)
        """
        # Convert actions back to pre-tanh values if not provided
        if pre_tanh_value is None:
            # Clamp to avoid numerical overflow
            action_clamped = torch.clamp(action, -1 + self.eps, 1 - self.eps)
            pre_tanh_value = 0.5 * (
                torch.log1p(action_clamped) - torch.log1p(-action_clamped)
            )  # atanh

        # Gaussian log_prob of pre-tanh value
        logp = self.normal.log_prob(pre_tanh_value)

        # Correction term from tanh transformation (Jacobian)
        logp -= torch.log(1 - action.pow(2) + self.eps)

        return logp.sum(-1)  # sum over action dims
