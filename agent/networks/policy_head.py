...
class DeterministicHead(nn.Module):
    ...
    def loss_re_fn(self, dist, target,retrieve_act, reduction="mean", **kwargs):
        act_log_probs = dist.log_prob(target)
        act_loss = -act_log_probs

        re_log_probs = dist.log_prob(retrieve_act)
        re_loss = -re_log_probs

        if reduction == "mean":
            act_loss = act_loss.mean() * self.loss_coef
            re_loss = re_loss.mean() * self.loss_coef
        elif reduction == "none":
            act_loss = act_loss * self.loss_coef
            re_loss = re_loss * self.loss_coef
        elif reduction == "sum":
            act_loss = act_loss.sum() * self.loss_coef
            re_loss = re_loss.sum() * self.loss_coef
        else:
            raise NotImplementedError
